...
def instantiate_from_config(config):
    if not "target" in config:
        if config == '__is_first_stage__':
            ...
            return None
        elif "frameinit_config" in config:
            return config['frameinit_config']
...
def freq_mix_3d(x, noise, LPF, low_freq_norm=True, norm_factor=1.0):
    ...
    # Normalize low-frequency components 
    if low_freq_norm:
        low_freq_magnitude = torch.abs(x_freq_low)
        mean_mag = low_freq_magnitude.mean()
        x_freq_low = (x_freq_low / (mean_mag + 1e-8)) * norm_factor
...
def get_freq_filter(shape, device, filter_type, n, d_s):
    """
    ...
    if filter_type == "gaussian":
        return gaussain_low_pass_filter1(shape=shape, d_s=d_s).to(device)
        # return gaussain_low_pass_filter(shape=shape, d_s=d_s, d_t=d_t).to(device)
...
def random_false_to_true(mask, change_ratio=0.2):

    false_indices = torch.nonzero(~mask, as_tuple=False)  
    n_change = int(len(false_indices) * change_ratio)     

    selected_indices = false_indices[torch.randperm(len(false_indices))[:n_change]]

    mask_with_changes = mask.clone()  
    for idx in selected_indices:
        mask_with_changes[idx[0], idx[1]] = True  

    return mask_with_changes

def set_refattention_adaptive_range(model, adaptive_ranges):
    from ldm.modules.attention import RefAttention
    """
    Set the adaptive range for RefAttention layers in the model.
    
    Args:
        model: The model containing RefAttention layers.
        adaptive_ranges: A dictionary mapping layer names to their adaptive ranges.
    """
    for name, module in model.named_modules():
        if isinstance(module, RefAttention):
            module.adaptive_range = adaptive_ranges
        else:
            continue
                # print(f"Warning: No adaptive range provided for {name}.")