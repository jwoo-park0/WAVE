...
def fov_from_intrinsics2(intrinsics):
...
def extract_matrix(warped_images):
    ...
    warped_images = torch.from_numpy(warped_images).permute(0,3,1,2)
    masks = (warped_images != 0).float() 

    n_images = warped_images.size(0)
    iou_matrix = torch.zeros((n_images, n_images))
    ...
    return iou_matrix_np



def find_significant_range_per_row(warped_images, threshold=0.05):
    

    iou_matrix = extract_matrix(warped_images)
    ranges = []
    all_abs_diffs = [] 

    for i in range(iou_matrix.shape[0]):  
        ref_value = iou_matrix[i, i]  
        abs_diff = np.abs(iou_matrix[i] - ref_value) 
        all_abs_diffs.append(abs_diff)  
        
        threshold = np.std(iou_matrix[i]) * 2
        significant_indices = np.where(abs_diff < threshold)[0]
        ...
        if len(significant_indices) > 0:
            ...
            ranges.append(iou_matrix.shape[1])
            # ranges.append((i, 0, iou_matrix.shape[1] - 1))
    return ranges, all_abs_diffs
