...
def get_ccw_poses(num_steps=10, radius=1, rotation_angle=30, zscale=1, xscale=1, c2w=True, endpoint=(0,-1)): #반시계 방향 
    extrinsics = []
    a,b = endpoint
    for step in range(num_steps):
...
def load_depth_model():
    ...
    with warnings.catch_warnings():
        ...
        model_config = {'vitl': {'encoder': 'vitl', 'features': 256, 'out_channels': [256, 512, 1024, 1024]}}
        depth_model = DepthAnything(model_config[encoder]).cuda().eval()
        depth_model.load_state_dict(torch.load(f'Depth-Anything/ckpt/depth_anything_{encoder}14.pth'))
